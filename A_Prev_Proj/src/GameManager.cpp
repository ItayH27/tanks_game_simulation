...
// Visualisation
void GameManager::writeBoardToJson() const {
...
// Visualisation
void GameManager::setVisualMode(const bool visual_mode) { this->visualMode_ = visual_mode; }

// Constructor for GameManager class
GameManager::GameManager(unique_ptr<PlayerFactory> player_factory, unique_ptr<TankAlgorithmFactory> tank_factory) : playerFactory_(
...
bool GameManager::extractLineValue(const std::string& line, int& value, const std::string& key, const size_t line_number) {
...
void GameManager::readBoard(const string& file_path) {
...
bool GameManager::failedInit() const {
    return failedInit_;
}

// Function to get actions for both tanks
void GameManager::getTankActions() {
    tankActions_.clear();
...
bool GameManager::isValidAction(const TankInfo& tank, const ActionRequest action) const {
...
bool GameManager::isValidMove(const TankInfo& tank, const ActionRequest action) const {
...
bool GameManager::isValidShoot(const TankInfo& tank) {
...
void GameManager::shoot(TankInfo& tank) {
...
void GameManager::moveTank(TankInfo& tank, const ActionRequest action) {
    // Get the current location of the tank
    auto [fst, snd] = tank.getLocation();
    const int x = fst;
    const int y = snd;
...
void GameManager::rotate(TankInfo& tank, const ActionRequest action) {
...
bool GameManager::performAction(const ActionRequest action, TankInfo& tank) {
...
void GameManager::performTankActions() {
...
void GameManager::checkTanksStatus() {
...
int GameManager::getTankIndexAt(int x, int y) const {
...
ShellIterator GameManager::getShellAt(const int x, const int y) {
...
ShellIterator GameManager::deleteShell(ShellIterator it) {
    if (it != shells_.end()) {
...
void GameManager::moveShells(vector<unique_ptr<Shell>>& shells) {
...
void GameManager::checkShellsCollide() {
...
pair<int, int> GameManager::nextLocation(const int x, const int y, const Direction dir, const bool backwards) const {
...
std::string GameManager::getEnumName(const Direction dir){
    switch (dir) {
...
std::string GameManager::getEnumName(const ActionRequest action) {
    switch (action) {
...
pair<int, int> GameManager::getGameboardSize() const {
    return {width_, height_};
}

void GameManager::updateGameLog() {
